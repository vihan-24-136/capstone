...
# Streamlit UI
st.set_page_config(page_title="AI Recipe Assistant", page_icon="🍽")
st.title("🍽 AI-Powered Kitchen Assistant")
...
# Main tabs
tab1, tab2, tab3, tab4 = st.tabs(["📋 Recipe Generator", "♻ Leftover Dish", "🍎 Nutrition Info", "🎉 Event Menu Planner"])
...
with tab2:
    st.header("♻ Create Dish from Leftovers")
    ...
    if st.button("Generate Event Menu", key="btn4"):
        ...
        with st.spinner("Planning your perfect event menu..."):
            try:
                model = genai.GenerativeModel(MODEL_NAME)
                response = model.generate_content(prompt)
                st.success(f"Here’s a perfect menu for your {event_type}:")
                st.write(response.text)
            except Exception as e:
